#!/usr/bin/env python3
"""Run one isolated book per size cluster, in parallel.
...
CAGR hides that completely. Each cluster gets its own Rs 5L book, its own
positions and its own drawdown, so the per-cluster result stands alone.
...
def _one(cluster):
    """Run a book that may only ever hold names from `cluster`."""
    clusters.CLUSTER_NAMES = NAMES
    r = simulate.run(_CORPUS, _DAYS, take_per_cluster={cluster: BASE["max_pos"]}, **BASE)
    by = defaultdict(float)
    for t in r["trades"]:
        by[_blk(t["day"])] += t["ret"]
    n = len(r["trades"])
    return {
        "cluster": cluster, "cagr": r["cagr"], "maxdd": r["maxdd"], "n": n,
...
def main(names=NAMES):
    ...
    clusters.CLUSTER_NAMES = names
    print(f"{len(_CORPUS)} symbols, {len(_DAYS)} sessions -> {len(names)} clusters "
    ...
    print(f"  {'cluster':<8}{'CAGR':>9}{'maxDD':>8}{'win':>6}{'n':>6}"
          f"{'avg/trade':>11}{'worst blk':>11}{'neg':>7}")
    for x in sorted(res, key=lambda y: -y["cagr"]):
        print(f"  {x['cluster']:<8}{x['cagr']:>+8.2f}%{x['maxdd']:>7.1f}%"
    ...
    for x in sorted(res, key=lambda y: -y["worst_block"]):
        print(f"    {x['cluster']:<8} worst {x['worst_block']:>+8.1f}%   "
              f"CAGR {x['cagr']:>+7.2f}%")

    kept = 0
    for x in res:
        if simulate.keep(f"cluster-only: {x['cluster']}", x["_r"],
                         {**BASE, "clusters": list(names), "only": x["cluster"]},
                         batch="cluster-split", track="cluster",
                         note=f"isolated {x['cluster']} book, {len(names)}-way split"):
            kept += 1
